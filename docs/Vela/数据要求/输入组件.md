...
> 项目适配说明（2026-02-04）
> - 当前项目使用自定义输入法组件 `src/components/InputMethod/InputMethod.ux`。
> - 已在 `pages/search/search.ux` 与 `pages/settings/settings.ux` 中接入；键盘类型为 QWERTY。
> - `maxlength` 在搜索页为 15，在设置页为 15；组件内部会限制最大 20。
![预览图](prew.png)
