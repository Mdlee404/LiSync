...
> 项目适配说明（2026-02-04）
> - 当前项目使用 `@system.interconnect` 与 Android 通信，协议见 `docs/基础通信`。
> - 构建命令见 `package.json`，签名与发布说明仍以本文档为参考。
...
**重要：除 `theme.file.chunk` 外，所有请求都要带 `_requestId`，响应必须原样回传 `_requestId`。**
> 手表端通过 `lisync.sendMessage` 发送时会自动补 `_requestId`，但仍建议手机端严格回传。
