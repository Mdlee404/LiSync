...
本文档为 Android 客户端开发人员提供详细的通信协议规范，用于开发与 **HeMusic** (Vela 快应用) 配套的 **LiSync** 安卓伴侣应用。
...
> 备注：手表端发送消息时若未提供 `_requestId`，会自动补一个，但仍建议 Android 端严格回传。
...
```
*   快应用解析优先级: `data` (if string) > `data.url` > `url`。建议统一放在 `data` 或 `url` 字段。
*   注意：`action` 字段可缺省（代码通过字段特征识别）。
...
**手表端行为说明（与代码一致）**：
- 保存路径固定为：`internal://files/music/{name}`
- 分片写入按收到顺序追加（不校验 `index` 顺序）
- 未做 `md5` 校验；若需校验请在 Android 端自行完成
...
`docs/基础通信/Theme_Transfer_Protocol.md`
