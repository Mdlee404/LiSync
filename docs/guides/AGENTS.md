...
* **主题传输**：选择 `theme.zip` 后自动解压到私有目录并按协议发送（主题 ID 统一小写）。
...
XiaomiWearableManager
  ...
  ├─ action=theme.\*  -> ThemeTransfer (open/init/file.start/chunk/finish/finish/cancel)
  └─ 其他请求       -> RequestProxy
...
|JSON 处理|Gson|2.10.1|
|缓存存储|JSON 文件 + 内存（CacheManager）|4 小时|
...
> 说明：Room 依赖仍在 gradle 中，但当前代码未使用 Room。
...
│       │   │   ├── MainActivity.java
│       │   │   ├── SearchPlayActivity.java
│       │   │   ├── SettingsActivity.java
│       │   │   └── ThemeTransferActivity.java
...
|ProGuard|不应用|未启用（`minifyEnabled false`，仅配置 proguardFiles）|
...
* 处理消息动作：
  ...
  * `theme.open` / `theme.init` / `theme.file.start` / `theme.file.chunk` / `theme.file.finish` / `theme.finish` / `theme.cancel`（主题传输协议）
...
* `targetScriptId`/强制脚本优先；`forcePolling=false` 时仅使用指定脚本，`forcePolling=true` 时优先指定脚本后继续轮询。
* 若脚本不支持音质，降级到支持列表中的最低优先级（默认顺序：`128k` → `320k` → `flac` → `flac24bit`）。
...
* 详细协议见：`docs/protocols/LiSync\_Protocol\_Spec.md` 与 `docs/protocols/vela\_quickapp\_integration.md`。
...
* `docs/protocols/LiSync\_Protocol\_Spec.md` - 通信协议规范
* `docs/protocols/Theme_Transfer_Protocol.md` - 主题传输协议
* `docs/protocols/vela\_quickapp\_integration.md` - 手表快应用接入指南
* `docs/theme/theme.md` - 主题包标准
* `docs/guides/custom\_source.md` - 自定义音源脚本说明
* `docs/guides/GEMINI.md` - 项目生态与参考说明
* `docs/plans/plan.md` - 项目实施计划
* `docs/references/sl.md` - 搜索/歌词逻辑参考
* `docs/references/interconnect测试.txt` - 互联互通测试
* `docs/references/小米穿戴第三方APP能力开放接入文档.txt` - SDK 接入文档
* `docs/references/小米穿戴第三方APP能力开放接口文档_1.4.txt` - SDK 接口文档
* `interconnect测试.txt` - 互联互通测试
